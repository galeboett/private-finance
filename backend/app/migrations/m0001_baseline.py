...
from .. import models  # noqa: F401 - registers every model with Base metadata
from ..db import Base


def upgrade(connection: Connection) -> None:
    """Create today's schema and upgrade databases produced before migrations."""
    ...
    if "entity_type" not in operation_change_columns:
        ...
        connection.execute(
            text(
                """
                UPDATE operation_changes
                SET entity_type = (
                  SELECT entity_type
                  FROM operations
                  WHERE operations.id = operation_changes.operation_id
                )
...
def migrate_category_rules_for_optional_category(connection: Connection) -> None:
    ...
    connection.execute(
        text(
            ...
            SELECT id, category_id, priority, field_name, match_text,
                   suggested_transaction_type, created_at, updated_at
            FROM category_rules
            """
        )
    )
