...
import { api as rawApi, apiUrl, bumpTransactionsVersion, getTransactionsVersion, parseApiJson, readableApiError, subscribeTransactionsVersion } from "../api/hooks";
...
import type { FinanceController } from "../controller/useFinanceController";
...
export function FinanceWorkspaceView({ controller }: { controller: FinanceController }) {
  ...
  const {
    ...
    handleLogout, handleTransactionRowClick, hasMoreTransactions, holdingRows, importAnalysis, isLoadingMoreTransactions,
      ...
      <main className={activeView === "settings" ? "workspace settingsWorkspaceView" : "workspace"}>
          ...
          <div className="ledgerListToolbar">
            <div className="ledgerSummaryGroup">
              <div className="ledgerSummaryText">
                <strong>{filteredTransactions.length} loaded transaction{filteredTransactions.length === 1 ? "" : "s"}</strong>
            ...
            {pagedTransactions.length < filteredTransactions.length ? (
              ...
              <button type="button" className="secondaryButton compactButton" disabled={isLoadingMoreTransactions} onClick={() => void loadMoreTransactions()}>
                {isLoadingMoreTransactions ? "Loading…" : "Load 200 more"}
              </button>
            ) : null}
          </div>
        ...
        {activeView === "settings" && settingsTab === "data" ? (
          <DataSettings
            ...
            onExport={(password, passphrase) => void downloadAppExport(password, passphrase)}
            onRestoreExport={(password, passphrase) => void restoreAppExport(password, passphrase)}
            onOpenTrash={() => {
