...
import { bumpTransactionsVersion, getTransactionsVersion, parseApiJson, readableApiError, subscribeTransactionsVersion, useApiClient, useApiFetch } from "../api/hooks";
import { readAppRoute, routeUrl, type RouteView } from "../app/router";
import { useAccountsNav } from "../domain/useAccountsNav";
import { useActivity } from "../domain/useActivity";
import { useImports } from "../domain/useImports";
import { useLedger } from "../domain/useLedger";
import { useNetWorth } from "../domain/useNetWorth";
import { useReviewQueue } from "../domain/useReviewQueue";
...
export function useFinanceController() {
  ...
  const domainEnabled = Boolean(csrf);
  const ledger = useLedger<TransactionRow>(transactionView, domainEnabled, transactionsVersion);
  const accountsNav = useAccountsNav<AccountSummary, DashboardSummary>(domainEnabled);
  const reviewQueueDomain = useReviewQueue<TransferCandidate, RefundSuggestionGroup, DuplicatePair, ReconciliationStatus, PaymentVerificationStatus>(domainEnabled);
  const importsDomain = useImports<ImportInboxState>(domainEnabled);
  const netWorthDomain = useNetWorth<NetWorthAccount, AllocationRow, HoldingRow>(domainEnabled);
  const activityDomain = useActivity<OperationSummary>(domainEnabled);
  ...
  useEffect(() => {
    if (!csrf) return;
    setTransactions(ledger.rows);
    setSelectedTransactionIds([]);
    setFocusedTransactionId(null);
    setEditingTransactionId(null);
  }, [csrf, ledger.rows, setSelectedTransactionIds]);

  useEffect(() => {
    if (accountsNav.accounts.data) setAccounts(accountsNav.accounts.data);
    if (accountsNav.dashboard.data) setDashboard(accountsNav.dashboard.data);
  }, [accountsNav.accounts.data, accountsNav.dashboard.data]);

  useEffect(() => {
    if (reviewQueueDomain.transfers.data) setTransferCandidates(reviewQueueDomain.transfers.data);
    if (reviewQueueDomain.refunds.data) setRefundSuggestions(reviewQueueDomain.refunds.data);
    if (reviewQueueDomain.duplicates.data) setDuplicatePairs(reviewQueueDomain.duplicates.data);
    if (reviewQueueDomain.reconciliation.data) setReconciliationStatuses(reviewQueueDomain.reconciliation.data);
    if (reviewQueueDomain.payments.data) setPaymentVerification(reviewQueueDomain.payments.data);
  }, [reviewQueueDomain.transfers.data, reviewQueueDomain.refunds.data, reviewQueueDomain.duplicates.data, reviewQueueDomain.reconciliation.data, reviewQueueDomain.payments.data]);

  useEffect(() => {
    if (importsDomain.inbox.data) setImportInbox(importsDomain.inbox.data);
  }, [importsDomain.inbox.data]);

  useEffect(() => {
    if (netWorthDomain.accounts.data) setNetWorthAccounts(netWorthDomain.accounts.data);
    if (netWorthDomain.allocation.data) setAllocationRows(netWorthDomain.allocation.data);
    if (netWorthDomain.holdings.data) setHoldingRows(netWorthDomain.holdings.data);
  }, [netWorthDomain.accounts.data, netWorthDomain.allocation.data, netWorthDomain.holdings.data]);

  useEffect(() => {
    if (activityDomain.operations.data) setOperations(activityDomain.operations.data);
  }, [activityDomain.operations.data]);
  ...
  async function loadCoreData() {
    await Promise.all([
      accountsNav.dashboard.refetch(),
      accountsNav.accounts.refetch(),
      ledger.refetch(),
    ]);
  }
  ...
  async function loadSecondaryData() {
    await Promise.allSettled([
      ...
      netWorthDomain.accounts.refetch(),
      netWorthDomain.allocation.refetch(),
      netWorthDomain.holdings.refetch(),
      reviewQueueDomain.transfers.refetch(),
      reviewQueueDomain.refunds.refetch(),
      reviewQueueDomain.duplicates.refetch(),
      reviewQueueDomain.reconciliation.refetch(),
      reviewQueueDomain.payments.refetch(),
      importsDomain.inbox.refetch(),
      activityDomain.operations.refetch(),
    ]);
  }
  ...
  async function openTransactionPeek(filter: TxnFilter, title: string) {
    try {
      ...
      const page = await api<{ items: TransactionRow[]; next_cursor: string | null }>(`/api/transactions?${encodeTxnFilter(normalizedFilter).toString()}`);
  ...
  async function reauthenticate(passwordForAction: string) {
    if (!passwordForAction) throw new Error("Enter your current password for this sensitive action.");
    await api("/api/reauthenticate", {
      method: "POST",
      headers: { "x-csrf-token": csrf },
      body: JSON.stringify({ password: passwordForAction }),
    });
  }

  async function downloadAppExport(passwordForAction: string, encryptionPassphrase?: string) {
    setToast(null);
    try {
      await reauthenticate(passwordForAction);
      const path = encryptionPassphrase ? "/api/exports/app-data.encrypted" : "/api/exports/app-data.json";
      const response = await apiFetch(path, encryptionPassphrase ? {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json", "x-csrf-token": csrf },
      ...
      link.href = url;
      link.download = `private-finance-app-data-${new Date().toISOString().slice(0, 10)}.${encryptionPassphrase ? "pfenc" : "json"}`;
  ...
  async function restoreAppExport(passwordForAction: string, encryptionPassphrase?: string) {
    setToast(null);
    if (!appImportFile) {
    ...
    if (encryptionPassphrase) form.append("passphrase", encryptionPassphrase);
  ...
  return {
    ...
    analyzedAccount, appImportFile, applyRule, applyRuleToTransaction, applySavedRule,
    ...
    handleLogout, handleTransactionRowClick, holdingRows, importAnalysis,
    ...
    hasMoreTransactions: ledger.hasMore, isLoadingMoreTransactions: ledger.isLoadingMore,
    lastInboxScan, lastSavedRule, latestCashFlowRows, linkManualRefund, loadData, loadMoreTransactions,
    ...
    pagedTransactions, peekDrawer, pendingRuleTransaction, periodCashFlowRows, periodCategoryTotals, previewRows,
    ...
    setEditingCategoryLabel, setEditingCategoryParentId, setEditingRule, setGenericCsvMapping, setImportModalOpen,
